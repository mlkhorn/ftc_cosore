...
# this code stopped working but I don't know why
...
# Storing Latitude and Longitude in winterdata_summary
```{r site-lat-long}
...
# Create a New Column for Your New Bar Graph
```{r combine-year-dataset-name}

winterdata_summary %>% 
  mutate( dataset_abrev = substr(dataset, 11, nchar(dataset) )) %>% 
  mutate( col_abrev = paste( year, dataset_abrev)) -> winterdata_summary

```


# Bar Graph With All Summary Data
```{r bar-graph-with-all-data}

winterdata_summary %>% 
  ggplot( aes ( x = col_abrev, y = annual_Rs, fill = IGBP)) +
  geom_bar( stat= "identity" ) + 
  theme_minimal() +
  labs(x = "Dataset",
       y = "Annual Soil Respiration (gC/m2/yr)",
       title = "Annual Soil Respiration"  ) +
...
winterdata_summary %>% 
  ggplot( aes ( x = col_abrev, y = winter_Rs, fill = IGBP)) +
  geom_bar( stat= "identity" ) + 
  theme_minimal() +
  labs(x = "Dataset",
       y = "Winter Soil Respiration (gC/m2/winter)",
       title = "Winter Soil Respiration"  ) +
  theme(axis.text.x = element_text( angle = 60, hjust = 1) ) 
  


winterdata_summary %>% 
  ggplot( aes ( x = col_abrev, y = winter_Rs_contrib, fill = IGBP)) +
  geom_bar( stat= "identity" ) + 
  theme_minimal() +
  labs(x = "Dataset",
       y = "Winter Contribution",
       title = "Winter Contribution"  ) +
  theme(axis.text.x = element_text( angle = 60, hjust = 1) ) 
